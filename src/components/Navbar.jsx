          ...
          <div className="hidden md:flex space-x-1">
            {navItems.map((item) => (
              <Link
                key={item.name}
                to={item.path}
                className={`px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 ease-out flex items-center space-x-1 ${isActive(item.path)
                  ? 'text-gray-900 bg-gray-50 border border-gray-200/60 shadow-xs'
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {navItems.map((item) => (
                <Link
                  ...
                  className={`block px-3 py-2 rounded-md text-base font-medium transition-colors duration-200 ease-out flex items-center space-x-2 ${isActive(item.path)
                    ? 'text-gray-900 bg-gray-50 border border-gray-200/60 shadow-xs'
                    : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50'
                    }`}
                >
